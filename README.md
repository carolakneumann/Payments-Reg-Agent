...
3. Hybrid retrieval. Pure semantic search fails more often than anyone admits, especially in specialized regulatory language. "ODFI" and "RDFI" are semantically similar in embedding space but represent opposite sides of an ACH transaction with different obligations under the Nacha rules. "Network" means something different in a Reg II routing context than in a Nacha context than in a card scheme context. I added a keyword layer and a simple document relationship graph to catch cross-references — Reg E and the Nacha rules both govern ACH error resolution and cite each other, and the complete answer to most disputes questions requires pulling from both.
